...
def _toNode(x: Node|np.ndarray|float) -> Node:
...
class Node:
    ...
    def propagateDerivatives(self):
        """
            Backpropagates the gradient of this node to its parents.
        """
        pass
...
class _Power(Node):
    def __init__(self, base_node: Node|np.ndarray|float, exp_node: Node|np.ndarray|float):
        base_node = _toNode(base_node)
        exp_node = _toNode(exp_node)
...
class _Concatenate(Node):
    def __init__(self, nodes: list[Node]|list[np.ndarray], axis: int=-1):
        for i in range(len(nodes)): nodes[i] = _toNode(nodes[i])
